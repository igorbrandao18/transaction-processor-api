"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.QueueMetricsService = void 0;
const common_1 = require("@nestjs/common");
const bull_1 = require("@nestjs/bull");
const metrics_config_1 = require("../config/metrics.config");
let QueueMetricsService = class QueueMetricsService {
    queue;
    constructor(queue) {
        this.queue = queue;
    }
    onModuleInit() {
        void this.updateQueueMetrics();
        setInterval(() => {
            void this.updateQueueMetrics();
        }, 5000);
    }
    async updateQueueMetrics() {
        try {
            const [waiting, active, completed, failed] = await Promise.all([
                this.queue.getWaitingCount(),
                this.queue.getActiveCount(),
                this.queue.getCompletedCount(),
                this.queue.getFailedCount(),
            ]);
            metrics_config_1.transactionsQueueSize.set({ state: 'waiting' }, waiting);
            metrics_config_1.transactionsQueueSize.set({ state: 'active' }, active);
            metrics_config_1.transactionsQueueSize.set({ state: 'completed' }, completed);
            metrics_config_1.transactionsQueueSize.set({ state: 'failed' }, failed);
        }
        catch (error) {
            console.error('Error updating queue metrics:', error);
        }
    }
};
exports.QueueMetricsService = QueueMetricsService;
exports.QueueMetricsService = QueueMetricsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, bull_1.InjectQueue)('transactions')),
    __metadata("design:paramtypes", [Object])
], QueueMetricsService);
//# sourceMappingURL=queue-metrics.service.js.map